...
export default function Home() {
  const [anwser, setAnwser] = useState(""); // 逐字添加的文本
  const [fullAnwser, setFullAnwser] = useState(ANWSER); // 完整文本
  const [currentAIndex, setCurrentAIndex] = useState(0); // 当前字符的索引
  ...
  useEffect(() => {
    if (printingQuestion) {
      const questionTimer = setTimeout(() => {
        if (currentQIndex < fullQuestion.length) {
          setQuestion((prevText) => prevText + fullQuestion.charAt(currentQIndex));
      ...
      const answerTimer = setTimeout(() => {
        if (currentAIndex < fullAnwser.length) {
          setAnwser((prevText) => prevText + fullAnwser.charAt(currentAIndex));
          setCurrentAIndex((prevIndex) => prevIndex + 1);
        } else {
  ...
  }, [currentAIndex, currentQIndex, fullAnwser, fullQuestion, printingQuestion]);
  ...
  return (
    <main className={styles.indexPage}>
      <div className={styles.contentWrap}>
        ...
        <p className={styles.theQuestioner}>
          <span className={styles.participants}>Nobody： </span>
          {question}
        </p>
        <p className={styles.theAnwser}>
          <span className={styles.participants}>GPT-3.5： </span>
          {anwser.split("\n").map((line, index) => (
